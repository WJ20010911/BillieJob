...
const workflowTypes: RecordType[] = [
  "JD_SNAPSHOT",
  "CHAT_SCREENSHOT",
...
function RatingStars({ rating }: { rating: number | null }) {
  ...
  return (
    <span
      className="tracking-[0.12em] text-amber-400"
      aria-label={rating ? `${rating.toFixed(1)} 分` : "暂无评分"}
    >
      {[1, 2, 3, 4, 5].map((star) => (
...
function ImpactBadge({
  ...
  impact: CompanyExternalProfile["items"][number]["impact"];
}) {
  const styles = {
...
export default function CompanyPageClient({
  ...
  useEffect(() => {
    const frame = window.requestAnimationFrame(() => {
      ...
      void fetch(`/api/ads?companyId=${company.id}`)
        .then((response) => (response.ok ? response.json() : null))
        .then((result) => {
          if (!result) return;
  ...
  const availableCities = [
    ...new Set(allRecords.map((record) => record.city).filter(Boolean)),
  ];
  const positionOf = (record: RecordData) =>
    record.position || record.actualPosition || "岗位未填写";
  const availablePositions = [...new Set(displayRecords.map(positionOf))];
  const positionRecords = selectedPosition
    ? displayRecords.filter((record) => positionOf(record) === selectedPosition)
    : displayRecords;
  const recordsByPosition = positionRecords.reduce<
  ...
  const contributorRows = (records: RecordData[]) =>
    Object.values(
      records.reduce<Record<string, RecordData[]>>((groups, record) => {
  ...
  const completeAd = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/ads", {
        ...
        body: JSON.stringify({ adId: ad.id, companyId: company.id }),
      });
      ...
      setAdError(error instanceof Error ? error.message : "广告解锁失败");
    } finally {
      setAdLoading(false);
    }
  };
    ...
    <div className="mx-auto max-w-5xl px-4 py-6">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-lg font-semibold text-slate-950">
                {company.ratingSummary.average
                  ? company.ratingSummary.average.toFixed(1)
                  : "暂无"}
        ...
        <div className="mb-6 border-y border-slate-100 py-4">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button
              ...
              className={`border px-3 py-1.5 text-sm transition ${!selectedPosition ? "border-slate-950 bg-slate-950 text-white" : "border-slate-200 bg-white text-slate-600 hover:border-slate-400"}`}
            >
              全部
            </button>
            {availablePositions.map((position) => (
              <button
                ...
                className={`border px-3 py-1.5 text-sm transition ${selectedPosition === position ? "border-slate-950 bg-slate-950 text-white" : "border-slate-200 bg-white text-slate-600 hover:border-slate-400"}`}
              >
                {position}
              </button>
            ))}
      ...
      {!showContent && positionRecords.length > 0 ? (
        <div className="mb-6 rounded-2xl border border-slate-200 bg-slate-50 p-8 text-center">
          ...
          <p className="mt-2 text-sm text-slate-500">
            看完广告后可继续查看全部 {positionRecords.length}{" "}
            条记录；会员可直接跳过。
          </p>
      ...
      <div className="space-y-4">
        {positionRecords.length === 0 ? (
          <div className="py-12 text-center text-slate-400">
            ...
            <a
              href="/upload"
              className="mt-3 inline-block text-sm text-slate-900 hover:underline"
            >
              去分享第一条经历
            </a>
          </div>
        ) : showContent ? (
          Object.entries(recordsByPosition).map(([position, records]) => (
            <section
              key={position}
            ...
            >
              ...
              <div className="overflow-x-auto border border-slate-200 bg-white">
                <div className="min-w-[860px]">
                    <div className="grid grid-cols-[150px_repeat(4,minmax(0,1fr))] border-b-2 border-slate-900 bg-slate-50 text-xs font-bold text-slate-600">
                    <div className="px-4 py-3">发布者 / 时间</div>
                  ...
                  {contributorRows(records).map((row, rowIndex) => {
                    const newest = row.reduce(
                      (latest, item) =>
                    ...
                    return (
                      <div
                        key={`${position}-${rowIndex}`}
                          className="grid grid-cols-[150px_repeat(4,minmax(0,1fr))] border-b border-slate-200 last:border-b-0"
                      >
                        <div className="bg-slate-50/60 px-4 py-4">
                          <p className="text-sm font-semibold text-slate-900">
                            贡献者 {rowIndex + 1}
                          </p>
                          <time className="mt-1 block text-xs leading-5 text-slate-500">
                            {formatPublishedAt(newest.createdAt)}
                          </time>
                        ...
                        {workflowTypes.map((type) => {
                          const record = row.find((item) => item.type === type);
                          return (
                            <div
                              key={type}
                              className="min-h-36 border-l border-slate-200 p-3"
                            >
                              {record ? (
                                <a
                                  href={`/records/${record.id}`}
                                  className="group block h-full border border-slate-200 bg-white p-3 transition hover:border-slate-950 hover:bg-slate-50"
                                >
                                  ...
                                  <p className="mt-2 line-clamp-2 text-sm font-semibold leading-5 text-slate-900 group-hover:text-cyan-800">
                                    {record.title}
                                  </p>
                                  <p className="mt-2 line-clamp-3 text-xs leading-5 text-slate-500">
                                    {record.content}
                                  </p>
                                  <time className="mt-3 block text-[11px] text-slate-400">
                                    {formatPublishedAt(record.createdAt)}
                                  </time>
                                </a>
                              ) : (
                                <div className="flex h-full min-h-28 items-center justify-center border border-dashed border-slate-200 bg-slate-50 text-xs text-slate-400">
                                  尚未发布
                                </div>
                              )}
                            </div>
      ...
      {showAd && ad ? (
        <div
          className="fixed inset-0 z-[200] flex items-center justify-center bg-slate-950/55 px-4 py-6"
          role="dialog"
          aria-modal="true"
          aria-label="推广内容"
        >
          <div className="w-full max-w-md border border-slate-200 bg-white shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4">
              ...
              <button
                type="button"
                onClick={() => void completeAd()}
                disabled={adLoading}
                className="h-8 w-8 text-xl text-slate-500 hover:bg-slate-100"
                aria-label="关闭广告"
              >
                ×
              </button>
            ...
            <div className="p-5">
              <p className="whitespace-pre-wrap text-sm leading-6 text-slate-600">
                {ad.description || "感谢查看推广内容。关闭后即可查看公司记录。"}
              </p>
              {ad.targetUrl ? (
                <a
                  href={ad.targetUrl}
              ...
              {adError ? (
                <p className="mt-3 text-sm text-red-600">{adError}</p>
              ) : null}
              <button
                type="button"
                onClick={() => void completeAd()}
                disabled={adLoading}
                className="mt-5 w-full bg-slate-950 px-4 py-3 text-sm font-bold text-white disabled:opacity-50"
              >
                {adLoading ? "解锁中..." : "关闭广告并查看记录"}
              </button>
            </div>
          </div>
        </div>
