...
const fieldGroups = [
  { title: "薪资总览", keys: ["salaryRange", "estimatedGross", "estimatedTakeHome", "afterTaxIncome"] },
  { title: "薪资构成与补贴", keys: ["salaryStructure", "salaryBase", "performance", "commission", "probationCompensation", "regularCompensation", "monthlyAllowance", "dailyAllowance", "mealAllowance", "transportAllowance", "housingAllowance", "bonus"] },
  { title: "工作制度", keys: ["workTime", "dailyHours", "weeklyWorkDays", "weeklyHours", "shiftWork", "overtimePolicy"] },
  { title: "岗位与用工", keys: ["duties", "requirements", "location", "taskRequirement", "benefits", "socialBenefits", "employment", "process"] },
];

const wideFields = new Set(["salaryRange", "salaryStructure", "probationCompensation", "regularCompensation", "estimatedGross", "estimatedTakeHome", "afterTaxIncome", "dailyAllowance", "duties", "requirements"]);
    ...
    <div className={screen === "result" ? "min-h-screen bg-slate-100" : "mx-auto max-w-6xl px-4 py-8 sm:px-6 sm:py-12"}>
      ...
      {result && screen === "result" ? <section className="w-full border-y-2 border-slate-900 bg-white shadow-[0_16px_42px_rgba(15,23,42,0.08)]">
        ...
        <div className="p-5 sm:p-7">
          ...
          {result.aiReview ? <section className={`mt-4 border p-4 ${result.aiReview.status === "complete" ? "border-violet-200 bg-violet-50/50" : "border-slate-200 bg-slate-50"}`}><div className="flex flex-col justify-between gap-2 sm:flex-row sm:items-center"><div><p className="text-xs font-bold uppercase tracking-[0.14em] text-violet-700">Dual analysis</p><h3 className="mt-1 text-base font-bold text-slate-950">规则引擎与 AI 复核</h3></div><span className={`w-fit px-2.5 py-1 text-xs font-bold ${result.aiReview.status === "complete" ? "border border-violet-200 bg-white text-violet-800" : "bg-slate-200 text-slate-600"}`}>{result.aiReview.status === "complete" ? "AI 复核已完成" : "AI 未返回，已使用规则结果"}</span></div>{result.aiReview.summary ? <p className="mt-3 text-sm leading-6 text-slate-700"><b>AI 摘要：</b>{result.aiReview.summary}</p> : null}{result.aiReview.status === "complete" ? result.aiReview.differences.length ? <div className="mt-3 overflow-x-auto"><p className="mb-2 text-xs font-semibold text-violet-800">以下字段两种分析不一致，请优先核对原图或原文：</p><table className="w-full min-w-[620px] text-left text-xs"><thead className="border-b border-violet-200 text-slate-500"><tr><th className="px-2 py-2">字段</th><th className="px-2 py-2">规则结果</th><th className="px-2 py-2">AI 原文判断</th></tr></thead><tbody>{result.aiReview.differences.map((item) => <tr key={item.fieldKey} className="border-b border-violet-100 align-top"><td className="px-2 py-2 font-semibold text-slate-800">{fieldLabels[item.fieldKey] || item.fieldKey}</td><td className="px-2 py-2 text-slate-700">{item.ruleValue}</td><td className="px-2 py-2 text-violet-800">{item.aiValue}</td></tr>)}</tbody></table></div> : <p className="mt-3 text-sm font-semibold text-emerald-700">AI 对可核验字段与规则结果没有发现需要人工确认的冲突。</p> : <p className="mt-3 text-xs leading-5 text-slate-500">{result.aiReview.summary || "请在管理员配置中填写可用的 AI 接口后启用第二路复核。"}</p>}</section> : null}
          ...
          <div className="mt-6 space-y-8">{fieldGroups.map((group) => <section key={group.title}><div className="mb-3 flex items-center gap-3"><h4 className="text-base font-bold text-slate-950">{group.title}</h4><div className="h-px flex-1 bg-slate-200" /></div><div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{group.keys.map((key) => { const field = result.fields[key] || { value: "", state: "missing" as const }; const tone = field.state === "found" ? "border-emerald-200 bg-emerald-50/50" : field.state === "unclear" ? "border-amber-200 bg-amber-50/50" : "border-slate-200 bg-slate-50"; const isWide = wideFields.has(key); const rows = key === "duties" || key === "requirements" ? 5 : isWide ? 3 : 2; return <label key={key} className={`block border p-3 ${tone} ${isWide ? "md:col-span-2 xl:col-span-2" : ""}`}><span className="flex items-center justify-between gap-2 text-xs font-bold text-slate-700">{fieldLabels[key]}<span className="shrink-0 text-[10px] font-medium text-slate-400">{savingField === key ? "保存中" : savedField === key ? "已保存" : field.state === "found" ? "已识别" : field.state === "unclear" ? "待核实" : "待补充"}</span></span><textarea value={editableValue(field)} onChange={(event) => updateField(key, event.target.value)} onBlur={() => void saveField(key)} rows={rows} placeholder="点击补充或修改" className="mt-2 block w-full resize-y border border-slate-300 bg-white px-2.5 py-2 text-sm leading-5 text-slate-800 outline-none focus:border-cyan-600" /></label>; })}</div></section>)}</div>
        </div>
      </section> : null}
