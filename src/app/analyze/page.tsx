    ...
    <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 sm:py-12">
      ...
      <main className="mt-8 grid gap-8 lg:grid-cols-[minmax(0,1fr)_340px]">
        ...
        <aside className="space-y-5">
          ...
          {result ? <section className="border-2 border-slate-200 bg-white p-5"><h2 className="text-base font-bold text-slate-950">信息字段状态</h2><div className="mt-3 space-y-2">{Object.keys(fieldLabels).map((key) => { const field = result.fields[key] || { value: "未在文字中提及", state: "missing" as const }; return <div key={key} className="flex items-start justify-between gap-3 border-b border-slate-100 py-2 text-sm"><span className="w-28 shrink-0 text-slate-600">{fieldLabels[key]}</span><span className={"min-w-0 break-words text-right " + (field.state === "found" ? "font-semibold text-emerald-700" : field.state === "unclear" ? "font-semibold text-amber-700" : "font-semibold text-red-600")}>{field.value}</span></div>; })}</div></section> : null}
        </aside>
      </main>
