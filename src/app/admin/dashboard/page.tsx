...
interface ImportedBackup { archiveName: string; archiveBytes: number; uploadedAt: string }
...
export default function AdminDashboard() {
  ...
  const [importedBackups, setImportedBackups] = useState<ImportedBackup[]>([]);
  ...
  const loadBackups = async () => {
    ...
    try {
      ...
      setImportedBackups(data.importedBackups || []);
  ...
  const uploadBackup = async () => {
    if (!backupFile) { setBackupMessage("请先选择 .tar.gz 备份包。"); return; }
    setBackupUploading(true); setBackupMessage("");
    try {
      const form = new FormData(); form.set("action", "upload"); form.set("backup", backupFile);
      const response = await fetch("/api/admin/backups", { method: "POST", body: form });
      const data = await response.json(); if (!response.ok) throw new Error(data.error || "备份包上传失败");
      setBackupFile(null); setBackupMessage("备份包校验通过，已放入待恢复列表。"); void loadBackups();
    } catch (error) { setBackupMessage(error instanceof Error ? error.message : "备份包上传失败"); }
    finally { setBackupUploading(false); }
  };

  const restoreBackup = async (archiveName: string) => {
    if (!window.confirm("恢复会替换当前的公司、用户、记录、会员、兑换码等全部数据库数据，并恢复附件。系统会先自动备份当前数据，随后短暂重启。确定继续吗？")) return;
    setBackupMessage("");
    try {
      const response = await fetch("/api/admin/backups", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "restore", archiveName }) });
      const data = await response.json(); if (!response.ok) throw new Error(data.error || "无法开始恢复");
      setBackupMessage(data.message || "恢复任务已开始，请等待页面重新连接。");
    } catch (error) { setBackupMessage(error instanceof Error ? error.message : "无法开始恢复"); }
  };
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8">
      ...
      {activeSection === "backup" ? <section className="flex flex-col space-y-4"><div className="border-2 border-slate-900 bg-white p-5"><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-start"><div><h2 className="text-lg font-bold text-slate-950">数据库与附件备份</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">备份包包含 SQLite 数据库快照、用户上传图片和还原清单。服务器每小时创建一次，默认保留最近 {backupRetentionHours} 小时；下载后可在新服务器直接恢复。</p></div><button type="button" onClick={() => void createBackup()} disabled={backupCreating} className="shrink-0 border-2 border-slate-950 bg-cyan-600 px-4 py-2 text-sm font-bold text-white disabled:opacity-50">{backupCreating ? "正在打包..." : "立即创建备份"}</button></div>{backupMessage ? <p className="mt-4 border border-cyan-200 bg-cyan-50 px-3 py-2 text-sm text-cyan-900">{backupMessage}</p> : null}</div><div className="border-2 border-amber-500 bg-amber-50 p-5"><h3 className="font-bold text-slate-950">上传并恢复备份包</h3><p className="mt-1 text-sm text-slate-600">仅接受本站下载的 .tar.gz 备份包。上传后先校验，再由你确认是否恢复。</p><div className="mt-4 flex flex-wrap items-center gap-3"><input type="file" accept=".tar.gz,application/gzip" onChange={(event) => setBackupFile(event.target.files?.[0] || null)} className="max-w-full text-sm" /><button type="button" onClick={() => void uploadBackup()} disabled={backupUploading || !backupFile} className="border-2 border-slate-950 bg-amber-400 px-4 py-2 text-sm font-bold text-slate-950 disabled:opacity-50">{backupUploading ? "上传校验中..." : "上传并校验"}</button></div>{importedBackups.length > 0 ? <div className="mt-5 divide-y border border-amber-200 bg-white">{importedBackups.map((backup) => <div key={backup.archiveName} className="flex flex-wrap items-center justify-between gap-3 px-4 py-3"><div><p className="text-sm font-semibold text-slate-900">{new Date(backup.uploadedAt).toLocaleString("zh-CN")} 上传</p><p className="text-xs text-slate-500">{formatBytes(backup.archiveBytes)} · 已通过包结构校验</p></div><button type="button" onClick={() => void restoreBackup(backup.archiveName)} className="border-2 border-red-700 bg-red-600 px-3 py-2 text-xs font-bold text-white">恢复此备份</button></div>)}</div> : <p className="mt-4 text-sm text-slate-500">尚未上传待恢复的备份包。</p>}</div><div className="overflow-x-auto border border-slate-200 bg-white"><div className="flex items-center justify-between border-b border-slate-200 px-5 py-3"><h3 className="font-bold text-slate-950">最近备份</h3><button type="button" onClick={() => void loadBackups()} disabled={backupLoading} className="text-xs font-bold text-cyan-700 disabled:opacity-50">{backupLoading ? "刷新中..." : "刷新"}</button></div>{backups.length === 0 ? <p className="px-5 py-10 text-center text-sm text-slate-400">尚未找到备份。定时任务运行后或点击“立即创建备份”即可生成。</p> : <table className="w-full min-w-[720px] text-left text-sm"><thead className="bg-slate-50 text-xs text-slate-500"><tr><th className="px-5 py-3">创建时间</th><th className="px-5 py-3">备份内容</th><th className="px-5 py-3">压缩包大小</th><th className="px-5 py-3"></th></tr></thead><tbody>{backups.map((backup) => <tr key={backup.archiveName} className="border-t border-slate-100"><td className="px-5 py-4 text-slate-700">{new Date(backup.createdAt).toLocaleString("zh-CN")}</td><td className="px-5 py-4 text-slate-600">数据库 {formatBytes(backup.databaseBytes)} · {backup.uploadsIncluded ? `${backup.uploadFileCount} 个附件` : "无附件"}</td><td className="px-5 py-4 font-medium text-slate-900">{formatBytes(backup.archiveBytes)}</td><td className="px-5 py-4 text-right"><a href={`/api/admin/backups?download=${encodeURIComponent(backup.archiveName)}`} className="border border-cyan-300 px-3 py-2 text-xs font-bold text-cyan-800 hover:bg-cyan-50">下载</a></td></tr>)}</tbody></table>}</div></section> : null}
